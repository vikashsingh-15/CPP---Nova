...
float AverageCarPrice(std::vector<data>& values){
    ...
    */
    ...
    for(int index=0;index < Length; index++){
        if (values[index].find("Price") != values[index].end())
        {
...
int MaxHorsepower(std::vector<data>& values){
    ...
    for(int index=0;index < Length; index++){
         if (values[index].find("HorsePower") != values[index].end())
        {
            if (values[index].find("Price") != values[index].end())
            {
                int Price = values[index]["Price"];
                if (Price != -1)
                {
                    Power = values[index]["HorsePower"];
...
int FindLowestEfficiency(std::vector<data>& values){
    ...
    */
    ...
    for(int index=0;index < Length; index++){
        if (values[index].find("Efficiency") != values[index].end())
        {
            efficiency = values[index]["Efficiency"];
            if (efficiency != -1)
                LowestEfficiency = (LowestEfficiency > efficiency) ? efficiency : LowestEfficiency;
        }
